  ...
  shinyApp(
    ...
    server = function(input, output, session) {
      ...
      observeEvent(input$confirm_session_setup, {
        ...
        res <- df_full$data %>%
          ...
          filter(detection_score >= input$min_score) %>%
    ...
    #   # })
      ...
      observe({
        ...
        if (rec_end > df_template()$template_duration) {
          pos_silence <- rec_end - df_template()$template_duration
          rec_end <- df_template()$template_duration
        }
      ...
      observeEvent(input$get_templ_pars, {
        ...
        min_freq <- (df_template()$min_freq - 1) %>%
          round(0) %>%
          ifelse(. < 0, 0, .)
        # update spectrogram maximum frequency
        max_freq <- (df_template()$max_freq + 1) %>%
          round(0) %>%
          ifelse(. > 23, 23, .)
        # update the frequency band
        updateSliderInput(
          session, "zoom_freq", "Frequency band (kHz):",
          min = 0, max = (min(df_cut()$sample_rate) / 2000),
      ...
      observeEvent(input$default_pars, {
        ...
        updateSliderInput(
          session, "zoom_freq",
          min = 0, max = (det_i()$sample_rate / 2000),
          step = 1, value = c(0, 10)
        )
      })
      ...
      spectro_template <- reactive({
        ...
        # efficient_spectro(
          ...
          annotate(
            ...
            ymin = df_template()$min_freq,
            ymax = df_template()$max_freq,
      ...
      observe({
        ...
        if (file.exists(det_i()$soundscape_path) & input$wav_player_type == "HTML player") {
          ...
          temp_file <- tempfile(
            pattern = "detection_", tmpdir = here("temp"), fileext = ".wav"
          ) %>%
            gsub("\\\\", "/", .)
          seewave::savewav(res, f = res@samp.rate, filename = temp_file)
          ...
          if (length(wav_path) == 1) {
            ...
            if (file.exists(wav_path)) {
              if (input$wav_player_type == "HTML player") {
                ...
                temp_file <- tempfile(
                  pattern = "soundscape_", tmpdir = here("temp"), fileext = ".wav"
                ) %>%
                  gsub("\\\\", "/", .)
                seewave::savewav(res, f = res@samp.rate, filename = temp_file)
    ...
    observeEvent(input$hotkeys, {
      ...
      diag_input_procFUN <- function(x) {
        res <- x %>%
          select(template_name, detection_score, validation) %>%
          filter(
            template_name == input$template_name &
      ...
      output$res_table <- renderDT(
        {
          ...
          df_cut() %>%
            select(
              ...
              max_freq, detection_score, validation_user,
            ...
            formatRound(c("detection_start", "detection_end", "detection_score"), 3) %>%
      ...
      observe({
        ...
        if (nrow(df_diag_input()) > 2) {
          if (length(unique(df_diag_input()$validation)) == 2) {
            bin_mod <- glm(
              validation_bin ~ detection_score,
            ...
            if (input$diag_method == "Manual") {
              ...
              binom_cut <- data.frame(detection_score = seq(0, 1, 0.001)) %>%
                mutate(
                  prob = predict(bin_mod, newdata = ., type = "response")
                ) %>%
                {
                  .$detection_score[min(which(.$prob >= 0.95))]
                }
              updateSliderInput(session, "diag_cut", value = binom_cut)
            } else if (input$diag_method == "Error = 0.1") {
              binom_cut <- data.frame(detection_score = seq(0, 1, 0.01)) %>%
                mutate(
                  prob = predict(bin_mod, newdata = ., type = "response")
                ) %>%
                {
                  .$detection_score[min(which(.$prob >= 0.90))]
                }
              updateSliderInput(session, "diag_cut", value = binom_cut)
            } else {
              binom_cut <- NULL
            ...
            cutpointr_raw <- cutpointr(
              df_diag_input(), detection_score, validation,
            ...
            diag_out <- cutpointr_raw$roc_curve[[1]] %>%
              rename(detection_score = x.sorted) %>%
              mutate(
                template_name = input$template_name,
                precision = tp / (tp + fp),
                recall = tp / (tp + fn),
                ...
                selected = FALSE
                # selected = ifelse(detection_score >= input$diag_cut, TRUE, FALSE)
              ) %>%
              relocate(contains("template"), everything()) %>%
              as.data.frame()

            diag_out$selected[max(which(diag_out$detection_score > input$diag_cut))] <- TRUE
            ...
            precrec_data <- diag_out %>%
              select(detection_score, precision, recall, selected) %>%
              filter(is.finite(detection_score)) %>%
              tidyr::drop_na()
      ...
      diag_tab_default_filename <- reactive({
        ...
        res <- paste0(
          ...
          "min_score", round(cut_i_tab()$detection_score, 3), ".csv"
        )
      })
      observe({
