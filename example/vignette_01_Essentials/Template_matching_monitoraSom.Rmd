...
launch_validation_app(
  ...
  wl = 512, ovlp = 70, time_pads = 0.5,
  nav_autosave = TRUE, overwrite = TRUE, auto_next = TRUE
)
```
...
df_detecs_val_manual <- read.csv("validation_outputs/df_detecs_val_manual.csv")
...
If we perform the validation diagsnotics again, we see that the FN do interfere with the evaluation of the template matching process.
```{r}
# compute the diagnostics
ls_validations_tovlp <- diagnostic_validations_n(
  df_validated = df_detecs_val_tovlp,
  diag_method = "auto", pos_prob = 0.5
)
...
We notice that although the diagsnotics of the

While flexible, the validation by overlap with the validation app does not account for false negatives and although it is optimized for large scale use, users can still introduce errors due to observer biases. Thus, we recommend reserving it for exploratory analyses or validation under severely limited data availability.
